...
void MainWindow::onActualCancel()
{
    ...
    ui->pushButton_2->setEnabled(false); // prevent multi-clicks
    appendLog("Cancellation requested...");

    // Ask both managers to stop. Use QueuedConnection so stop() runs in manager threads.
    if (m_manager) {
